__kernel void labelPixels(__global unsigned char *data, 
...
__kernel void update_centers(__global unsigned char *data, 
                             __global double *centers, 
                             __global int *labels, 
                             __global double *dists, 
    ...
    //Calculating sums
    for (int px = item_offset; px < (item_offset + window) && px < n_px; px++) {
		 if (lid == labels[px]) {
            for (ch = 0; ch < n_ch; ch++) {
